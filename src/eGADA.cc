...
eGADA::eGADA(int _argc, char *_argv[]) : argc(_argc), argv(_argv) {
    ...
    examplesDoc = boost::format(
                      "%1% -i /tmp/input.tsv.gz -o /tmp/output.tsv.gz -M 1000 "
...
void eGADA::constructOptionDescriptionStructure()
{
    optionDescription.add_options()
        ...
        ("TBackElim,T", po::value<double>(&T)->default_value(5.0),
            "Minimal T-stat during the backward elimination of breakpoints. "
            "T-stat = (mean1-mean2)/stddev between two adjacent segments.")
        ("aAlpha,a", po::value<double>(&a)->default_value(0.5),
            "SBL hyper prior parameter for a breakpoint. It is "
            "the shape parameter of the Gamma distribution. Higher "
            "(lower) value means less (more) breakpoints.")
        ...
        ("convergenceDelta",
            ...
            "A delta number controlling convergence in the EM algorithm")
        ("maxNoOfIterations",
            po::value<long>(&maxNoOfIterations)->default_value(50000),
            "The maximum number of iterations before the EM convergence algorithm is "
            "stopped.")
        ("convergenceMaxAlpha",
        ...
        ("convergenceB", po::value<double>(&convergenceB)->default_value(1E-20),
            "one convergence related number.")
        ("debug,b", "Toggle debug/verbose mode for more status output")
        ("report,r", "Toggle report mode")
        ("reportIntervalDuringBE",
            po::value<int>(&reportIntervalDuringBE)->default_value(100000),
            "How often to report any break point removed during backward "
            "elimination.")
        ("inputFname,i", po::value<string>(&inputFname),
            "Input file path. It could be specified as "
            "an option or positional argument. "
            "If the suffix is .gz, the software will unzip it upon reading. "
            "It is a single-column text file with no header.")
        ("outputFname,o", po::value<string>(&outputFname), 
            "Output filepath. "
            "If the suffix is .gz, the software will zip the output automatically.");
}
