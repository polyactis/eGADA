...
long BaseGADA::SBLandBE() {
    ...
    */
    ...
    noOfBreakpointsAfterSBL = K;	// K would be changed later on.
    //Freeing memory
    myFree(alpha);
    //myFree(tn);	# commented out
    myFree(aux);
...
/******************************************************/
//BEwTscore(Iext,Wext,h0,h1,tscore,&K,T); 
//Need to update BEthres to operate on the Iext Wext notation...
long BaseGADA::BEwTscore(double *Wext, //IO Breakpoint weights extended notation...
    ...
    BreakPoint *leftMostBreakPointPtr = new BreakPoint(Iext[0], Wext[0], 
        tscore[0], 0, MinSegLen, T, Iext[K+1]);
    leftMostBreakPointPtr->nodePtr = rbTree.nil;
    BreakPoint *rightMostBreakPointPtr = new BreakPoint(Iext[K+1], 0, 0, 0, 
        MinSegLen, T, Iext[K+1]);
    rightMostBreakPointPtr->nodePtr = rbTree.nil;
    int maxBPSetSize =0;
    for (i = 1; i < K + 1; i++){
        //shorter of the two neighboring segments as length for the breakpoint
        long segLength = min(Iext[i]-Iext[i-1],Iext[i+1]-Iext[i]);
        BreakPoint* bpPtr = new BreakPoint(Iext[i], Wext[i], tscore[i], 
    ...
    long toRemoveSegmentLength=-1;	// initial value =-1, so that it is less than MinSegLen.
    ...
    while (rbTree.noOfNodes()>0 && (currentMinScore<T || toRemoveSegmentLength<MinSegLen)){
        ...
        for (setOfBPIterator =(*setOfBPPtr).begin(); setOfBPIterator!=(*setOfBPPtr).end(); 
            setOfBPIterator++){
            ...
            if (leftBreakPointPtr!=NULL &&  leftBreakPointPtr->nodePtr!=rbTree.nil && 
                leftBreakPointPtr->nodePtr!=NULL){
                ...
                if (rbTree.isNULLNode(genomeLeftNodePtr)){
                    //create an new node
                    genomeLeftNodePtr = rbTree.insertNode(leftBreakPointPtr->getKey(), 
                        new rbNodeDataType() );
                }
            ...
            if (rightBreakPointPtr!=NULL && 
                !rbTree.isNULLNode((rbNodeType*)rightBreakPointPtr->nodePtr) && 
                rightBreakPointPtr->nodePtr!=NULL){
                ...
                if (rbTree.isNULLNode(genomeRightNodePtr)){
                    //create an new node
                    genomeRightNodePtr = rbTree.insertNode(rightBreakPointPtr->getKey(), 
                    new rbNodeDataType() );
                }
    ...
    while (rbTree.noOfNodes()>0){
        minNodePtr = rbTree.getMinimum();
        setOfBPPtr = minNodePtr->getDataPtr();
        for (setOfBPIterator =(*setOfBPPtr).begin(); 
            setOfBPIterator!=(*setOfBPPtr).end(); setOfBPIterator++){
    ...
    //sort all the breakpoints in the tree by chromosomal position, 
    // reconstruct Iext, Wext, tscore, pK
...
long BaseGADA::RemoveBreakpoint(double *Wext, long *Iext, double *tscore, long K, 
